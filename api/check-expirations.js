...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const docSnap of snapshot.docs) {
      ...
      if (!telegramId) {
        console.log(`Менеджер "${entry.regionalManager}" не найден или не имеет Telegram ID`);
        results.push({ id: entry.id, status: 'no_telegram_id' });
        continue;
      }

      // Формируем сообщение
      const message = `⚠️ Защита проекта завершена! ⚠️
